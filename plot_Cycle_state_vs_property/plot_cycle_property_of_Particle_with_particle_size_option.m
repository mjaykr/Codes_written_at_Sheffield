...
for i = 1:length(unique_orientations)
    ...
    if strcmpi(annotate_particle_size, 'Y') && ~isempty(particle_size_col)
        ...
        particle_sizes = particle_size_data(idx);
        
        % Smart text placement
        text_handles = text(x_data, y_data, cellstr(num2str(particle_sizes)), ...
            'FontSize', 10, 'HorizontalAlignment', 'left', 'VerticalAlignment', 'bottom');
        
        % Adjust text positions to avoid overlap
        adjust_text(text_handles, x_data, y_data, particle_sizes);
    end
end
...
ylabel([selected_property, ' (mN)'], 'FontSize', 22);
